...
export default function Item() {
  ...
  const [sellerData, setSellerData] = useState({} as Seller)
          ...
          <Container>
            <div className={style['contend-aside-holder']}>
              ...
              <div style={{ display: "flex", flex: "1", flexDirection: "column", marginLeft: "20px" }}>
                ...
                <div className='mt-4 border w-100 p-3' style={{ marginLeft: "0px" }}>

                    <div className='fw-bold' style={{ fontSize: "30px", fontWeight: "500", marginBottom: "1rem" }}>{sellerData.name}</div>
                    <div style={{ fontSize: "16px" }}>
                      <LocationOnIcon style={{ fontSize: "20px" , marginRight: "6px" }} />
                      
                     {sellerData.location?.address + ', ' + sellerData?.location?.country}</div>
                    <div className='d-flex my-2'>
                      <ThumbUpIcon style={{ fontSize: "20px", marginRight: "6px" }} />
                      <Rating style={{ maxWidth: 100 }}

                        value={sellerData.reviews?.reduce((a, b) => a + b.rating, 0) / sellerData.reviews?.length || 0}

                        readOnly={true} itemStyles={myStyles} />
                      {'\u00A0'}  {(sellerData.reviews?.reduce((a, b) => a + b.rating, 0) / sellerData.reviews?.length || 0).toFixed(1)}
                      {'\u00A0\u00A0'}
                      ({sellerData.reviews?.length} Reviews)
                    </div>

                    {/* <Link href={`/auction-house/${sellerData?.id}`}
