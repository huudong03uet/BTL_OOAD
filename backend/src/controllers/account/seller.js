...
class ProfileController extends ProfileService {
    ...
    get_role_data(dict) {
        return {
            "phone": dict.seller.phone,
            "email": dict.seller.email,
            "name": dict.seller.name,
        }
    }
    ...
    register = async (req, res) => {
        ...
        try {
            ...
            const {id, expiry, cvn, name_card } = req.body.card_info;
            const {country, address, city, state, postal} = req.body.location_info;
            let seller = await this.model.findOne({
                where: {
                    user_id: user_id
                }
            })
            ...
            seller = await this.model.create({
                name: name,
                email: email,
                phone: phone,
                desciption: description,
    ...
    get_seller_by_user_id = async (req, res) => {
        try {
            if (!check_required_field(req.params, ["user_id"])) {
                logger.error(`${statusCode.HTTP_400_BAD_REQUEST} Missing required fields.`);
                return res.status(statusCode.HTTP_400_BAD_REQUEST).json("Missing required fields.");
    ...
    handle_verification_seller = async (req, res) => {
        try {
            const { seller_id, status } = req.body;
            ...
            const seller = await this.model.findByPk(seller_id);
            if (!seller) {
                logger.error(`${statusCode.HTTP_404_NOT_FOUND} Seller not found.`);
                return res.status(statusCode.HTTP_404_NOT_FOUND).json("Seller not found.");
            }
