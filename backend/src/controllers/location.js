...
let find_or_create_location = async (
  country,
  address,
  city,
  state,
  ...
  try {
    const { x, y } = await get_coordinate(
      `${_remove_diacritics(city)}, ${_remove_diacritics(country)}`
    );
    ...
    Object.keys(locationData).forEach(
      (key) => locationData[key] === undefined && delete locationData[key]
    );
    ...
    logger.info(
      `Find or Create: Create: ${created}, [location: ${location.id}]`
    );
    return location;
  } catch (error) {
