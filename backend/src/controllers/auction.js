...
let get_all_product_of_auction = async (req, res) => {
    try {
        ...
        if (!products) {
            const auction = AuctionRoom.findByPk(auction_id);
            if (!auction) {
                logger.error(`${statusCode.HTTP_404_NOT_FOUND} [auction:${auction_id}]`)
...
module.exports = { create_auction, add_user, add_product, get_all_product_of_auction, find_product_in_auction }
