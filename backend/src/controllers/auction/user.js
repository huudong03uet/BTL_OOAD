...
class AuctionController extends AuctionService {
    service_auction_upcomming = async (req, res) => {
        try {
            if (!check_required_field(req.params, ["user_id"])) {
                logger.error(`${statusCode.HTTP_400_BAD_REQUEST} Missing required fields.`);
                return res.status(statusCode.HTTP_400_BAD_REQUEST).json("Missing required fields.");
            ...
            const auctions = await this.get_auction(where_case)
    ...
    service_auction_promote = async (req, res) => {
        try {
            if (!check_required_field(req.params, ["user_id"])) {
                logger.error(`${statusCode.HTTP_400_BAD_REQUEST} Missing required fields.`);
                return res.status(statusCode.HTTP_400_BAD_REQUEST).json("Missing required fields.");
            ...
            let include = this.include.map(include => {
                if (include.model === Product) {
                    return {
                        ...include,
                        where: {
            ...
            const auctions = await this.get_auction(where_case, include)
    ...
    service_product_in_auction = async (req, res) => {
        try {
            if (!check_required_field(req.params, ["auction_id", "user_id"])) {
                logger.error(`${statusCode.HTTP_400_BAD_REQUEST} Missing required fields.`);
                return res.status(statusCode.HTTP_400_BAD_REQUEST).json("Missing required fields.");
    ...
    service_auction_bid = async (req, res) => {
        try {
            if (!check_required_field(req.params, ["product_id"])) {
                logger.error(`${statusCode.HTTP_400_BAD_REQUEST} Missing required fields.`);
                return res.status(statusCode.HTTP_400_BAD_REQUEST).json("Missing required fields.");
